# MLOPs
This is a test repository for the MLOPs session



# 🚀 Git Setup & Push to GitHub from Scratch

```bash
# Step 1: Create and navigate to your project folder
mkdir MyProject
cd MyProject

# Step 2: Initialize Git
git init

# Step 3: (Optional) Create a README file
echo "# MyProject" > README.md

# Step 4: Add all files to staging
git add .

# Step 5: Commit the files
git commit -m "Initial commit"

# Step 6: Add GitHub repository as remote
git remote add origin https://github.com/<your-username>/<repo-name>.git

# Step 7: Rename default branch to 'main' (if not already)
git branch -M main

# Step 8: Push to GitHub
git push -u origin main



# updating first time in vs code

git init   ------> Do if u aredoing from scratch dont do it if  u have cloned
git clone <URL> -----> To get a repository from github
git status      -----> Tells if u have modified something in that repository
git add .       -----> Makes everything ready  to push to github
git status      -----> After doing git add . it will show everything is ready


git commit -m "Any text U want"  ------>  After this it is ready to push
git status 

git push origin main        -----> Pushed it to github






git branch mlops-test   ---> mAkes a new branch of mlops-test
git checkout mlops-test  ----> Switch to that brach 
now all basics
git add . 
git status 
git commit -m "ANYTHING you have done"
git push origin mlops-test



